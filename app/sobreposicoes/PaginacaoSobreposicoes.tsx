...
function montarHref(pagina: number, revisadas: boolean) {
...
export function PaginacaoSobreposicoes({
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-black/5 bg-surface px-4 py-3 shadow-card">
      <Link
        href={montarHref(Math.max(1, paginaAtual - 1), revisadas)}
      ...
      <Link
        href={montarHref(Math.min(totalPaginas, paginaAtual + 1), revisadas)}
